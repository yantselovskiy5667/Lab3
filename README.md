...
1. Тест на `Добавление блюда`:
 ...
 2. Тест на `Распечатка меню`:
    ...
    - **Output**:
        ```
       Бургер (7x порций): 340.00 руб.
       Паста (4x порций): 380.00 руб.
       Пицца (5x порций): 450.00 руб.
       Салат (3x порций): 320.00 руб.
       Стейк (2x порций): 890.00 руб.
       Суп (6x порций): 280.00 руб.
        ```

3. Тест на `Распечатка доступных блюд`:
    - **Input**:
        ```
        Создаем меню на 6 блюд
      Добавляем: Пицца 450р 5порций 
    ...
    - **Output**:
       ...
       Суп (6x порций)
        ```
4. Тест на `Приготовление порций блюда`:
    - **Input**:
        ```
       uppPorcia(3, 4)
        ```
    ...
    - **Output**:
        ```
       Было: Салат (3x порций)  Стало: Салат (6x порций)
        ```
5. Тест на `Изменение цены блюда`:
    - **Input**:
        ```
       uppPrice(100,5)
        ```
    ...
    - **Output**:
        ```
       Было: Стейк : 890.00 руб.  Стало: Стейк : 990.00 руб.
        ```
